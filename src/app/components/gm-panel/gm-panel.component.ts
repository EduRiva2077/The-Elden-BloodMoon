  ...
  template: `
    <div class="w-80 h-full bg-stone-900 border-r border-stone-800 flex flex-col text-stone-300">
      ...
      <div class="flex border-b border-stone-800 text-xs font-mono">
        <button class="flex-1 py-3 transition-colors" [class.text-amber-500]="activeTab() === 'map'" [class.border-b-2]="activeTab() === 'map'" [class.border-amber-500]="activeTab() === 'map'" [class.bg-stone-800]="activeTab() === 'map'" (click)="activeTab.set('map')">Map</button>
      ...
      <div class="flex-1 overflow-auto relative">
        ...
        @if (activeTab() === 'map') {
          <div class="p-4 space-y-6">
            ...
            <div class="space-y-4">
              <div class="space-y-2">
                <h3 class="font-bold text-amber-500 border-b border-stone-700 pb-1">Map Background</h3>
                <div class="flex flex-col gap-2">
                  <div class="flex flex-col gap-1">
                    <label for="mapBgInput" class="text-xs text-stone-400">Image URL</label>
                  ...
                  <div class="flex flex-col gap-1">
                    <label for="mapBgUpload" class="text-xs text-stone-400">Or Upload Image</label>
        ...
        @if (activeTab() === 'tokens') {
          <div class="p-4 space-y-6">
            <!-- Add Token Settings -->
            <div class="space-y-2">
              <h3 class="font-bold text-amber-500 border-b border-stone-700 pb-1">Add New Token</h3>
              <div class="flex flex-col gap-2">
                <label for="newTokenType" class="text-xs text-stone-400">Token Type</label>
                <select id="newTokenType" #newTokenType class="bg-stone-800 border border-stone-700 rounded px-2 py-1 text-sm focus:outline-none focus:border-amber-500">
                  <option value="player">Player (Yellow Border)</option>
                  <option value="enemy">Enemy (Red Border)</option>
                  <option value="npc">NPC (Blue Border)</option>
                  <option value="boss">Boss (Black Border)</option>
                </select>
            ...
            <div class="space-y-2">
              <h3 class="font-bold text-amber-500 border-b border-stone-700 pb-1">Selected Token</h3>
              
              @if (selectedToken()) {
                <div class="space-y-3">
                  <div class="flex flex-col gap-1">
                    <label for="tokenNameInput" class="text-xs text-stone-400">Name</label>
                  ...
                  <div class="flex gap-2">
                    <div class="flex flex-col gap-1 flex-1">
                      <label for="tokenHpInput" class="text-xs text-stone-400">HP</label>
                    ...
                    <div class="flex flex-col gap-1 flex-1">
                      <label for="tokenMaxHpInput" class="text-xs text-stone-400">Max HP</label>
                  ...
                  <div class="flex flex-col gap-2">
                    <div class="flex flex-col gap-1">
                      <label for="tokenImgInput" class="text-xs text-stone-400">Image URL</label>
                    ...
                    <div class="flex flex-col gap-1">
                      <label for="tokenImgUpload" class="text-xs text-stone-400">Or Upload Image</label>
                  ...
                  <div class="flex flex-col gap-1">
                    <label for="tokenColorInput" class="text-xs text-stone-400">Color</label>
                  ...
                  <div class="pt-4 border-t border-stone-700 mt-4">
                    @if (tokenToDelete() === selectedToken()?.id) {
                      <div class="bg-red-900/30 border border-red-500/50 rounded p-3 text-sm flex flex-col gap-2">
                        <span class="text-red-400 font-bold text-xs">Are you sure you want to delete this token?</span>
                        <div class="flex gap-2">
                          <button class="flex-1 py-1 bg-red-600 hover:bg-red-500 text-white font-bold rounded text-xs transition-colors" (click)="confirmDeleteToken()">Yes, Delete</button>
                          <button class="flex-1 py-1 bg-stone-700 hover:bg-stone-600 text-white font-bold rounded text-xs transition-colors" (click)="tokenToDelete.set(null)">Cancel</button>
                        </div>
                      </div>
                    } @else {
                      <button class="w-full py-2 bg-red-900/20 hover:bg-red-900/50 border border-red-900/50 hover:border-red-500 text-red-400 hover:text-red-300 font-bold rounded text-xs transition-colors flex items-center justify-center gap-2"
                        ...
                        <mat-icon style="font-size: 16px; width: 16px; height: 16px;">delete</mat-icon> Delete Token
                      </button>
                    }
                  </div>
                </div>
                ...
                <div class="text-sm text-stone-500 italic p-4 text-center border border-dashed border-stone-700 rounded">
                  Select a token on the map to edit it.
                </div>
              }
            </div>
...
export class GmPanelComponent {
  ...
  addToken(type: string) {
    ...
    if (type === 'player') { name = 'New Player'; color = '#3b82f6'; }
    if (type === 'enemy') { name = 'New Enemy'; color = '#ef4444'; }
    if (type === 'npc') { name = 'New NPC'; color = '#22c55e'; }
    if (type === 'boss') { name = 'New Boss'; color = '#000000'; }
